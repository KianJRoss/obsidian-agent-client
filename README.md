...
<p align="center">
  <a href="README.ja.md">æ—¥æœ¬èªžã¯ã“ã¡ã‚‰</a>
</p>
...
Bring custom ACP agents directly into Obsidian. Chat with your AI assistant right from your vault.
...
- **Multi-Agent Support**: Switch between custom ACP agents
...
2. Go to **Settings â†’ BRAT â†’ Add Beta Plugin**
...
2. **Configure a custom ACP agent** in **Settings → Agent Client → Custom agents**:
   - **Node.js path**: e.g., `/usr/local/bin/node`
   - **Path**: absolute path to your custom ACP backend command
   - **Environment variables**: include credentials your backend expects, for example:
     - `GEMINI_API_KEY=...`
     - `OPENROUTER_API_KEY=...`

3. **Start chatting**: Click the robot icon in the ribbon
